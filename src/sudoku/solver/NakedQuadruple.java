...
public class NakedQuadruple implements SolvingStrategy {

	private static final long serialVersionUID = -7969250069814493779L;

	public NakedQuadruple() {
	...
	public boolean reduce(Sudoku sudoku, SolveLog log) {
		...
		for (Rule rule : sudoku.getRules()) {
			...
			for (int u1 = 0; u1 < cells.length; u1 += 2) {
				...
				if (c1.isSolved() || c1.getCandidates().size() > 4) {
					continue;
				}

				for (int u2 = u1 + 2; u2 < cells.length; u2 += 2) {
					Cell c2 = grid[cells[u2 + 1]][cells[u2]];
					if (c2.isSolved() || c2.getCandidates().size() > 4) {
						continue;
					}

					for (int u3 = u2 + 2; u3 < cells.length; u3 += 2) {
						Cell c3 = grid[cells[u3 + 1]][cells[u3]];
						if (c3.isSolved() || c3.getCandidates().size() > 4) {
							continue;
						}

						for (int u4 = u3 + 2; u4 < cells.length; u4 += 2) {
							Cell c4 = grid[cells[u4 + 1]][cells[u4]];
							if (c4.isSolved() || c4.getCandidates().size() > 4) {
								continue;
							}
							...
							Tools.addUnique(c4.getCandidates(), set);
							if (set.size() != 4) {
								continue;
							}
