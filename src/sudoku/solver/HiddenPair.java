...
public class HiddenPair implements SolvingStrategy {
	...
	private static final long serialVersionUID = 1310335335764647816L;
		...
		for (Rule rule : sudoku.getRules()) {
			...
			for (int c1 = 0; c1 < validInputs.length; c1++) {
				...
				for (int c2 = c1 + 1; c2 < validInputs.length; c2++) {
					...
					long mask = bitmasks[c1] | bitmasks[c2];
					if (Long.bitCount(mask) != 2) {
						continue;
					}
					char[] valid = { validInputs[c1], validInputs[c2] };
					String validstr = valid[0] + "/" + valid[1];
...
//									System.out.println("[DEBUG] cell R" + (rw + 1) + "C" + (cl + 1) + ": " + cell.getInnerMarks() + " <-> "
//											+ Arrays.toString(cell.getAvailables()) + " <-> " + c);
									reduced = true;
								}
					...
					if (reduced) {
						success = true;
						log.logStep("Hidden Pair %s in {%s} of %s", //
